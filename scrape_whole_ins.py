"""
금융감독원 보험다모아 종신보험 스크래퍼
http://203.229.168.79/wholeIns/wholeInsList.knia

Playwright로 세션 획득 후, HTTP 직접 요청으로 10건씩 병렬 엑셀 다운로드.

사용법:
    python scrape_whole_ins.py [--age-start 20] [--age-end 64] [--batch 10] [--output data/whole_ins]
"""
...
def get_filename(age, gender_name):
    return f"whole_{age}세_{gender_name}.xls"
...
def build_download_params(age, gender):
    ...
    return {
        "filename": urllib.parse.quote("종신보험", encoding="utf-8"),
        "queryId": "wholeIns.selectWholeInsExcelList",
...
async def download_one(session, age, gender, output_dir, cookie_str, semaphore):
    ...
    headers = {
        ...
        "Referer": "http://203.229.168.79/wholeIns/wholeInsList.knia",
    }
...
async def main():
    parser = argparse.ArgumentParser(description="금감원 보험다모아 종신보험 스크래퍼")
    ...
    parser.add_argument("--output", type=str, default="data/whole_ins", help="저장 폴더")
